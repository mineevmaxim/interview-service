...
function combineLongDelayInput(firstChange, secondChange) {
    ...
    if (firstChange.delayDuration >= minOperationDelay && isLongDelayInput(secondChange)) {
        longDelayAverage =
            (longDelayAverage * longDelayCount + secondChange.delayDuration) / (longDelayCount + 1);
        longDelayCount++;
        return true;
    }
    longDelayCount = 0;
...
function areInputPositionsContinue(firstChange, secondChange) {
    for (let i = 0; i < secondChange.ops.length; i++) {
        ...
        if (
            secondOp.from.line !== secondOp.to.line ||
            ...
            !(firstOp.from.line + 1 === secondOp.from.line && secondOp.from.ch === 0)
        ) {
            // For new line
            return false;
        }
